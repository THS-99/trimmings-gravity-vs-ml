...
def main():
    ...
    winner = sig["best_ml"]
    params = eval(tuning["best_params"][winner])
    df = feats.load_panel()
    ...
    if True:
        ...
        if name != "LGBM":
            lgb_params = eval(json.loads((RES/"ml_tuning_report.json").read_text())
                              ["best_params"][f"LGBM_{fs}"])
            train, test = feats.split(df, feats.ORIGINS[2])
            Xtr2 = feats.design_matrix(train, fs == "lags")
        ...
        try:
            ...
            plt.title(f"SHAP summary, LGBM_{winner.rsplit('_',1)[1]} (direction model), origin O3 test (2025)")
