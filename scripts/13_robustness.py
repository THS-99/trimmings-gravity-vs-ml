...
def fit_ml(train, test, winner, params, drop_cols=None):
    from sklearn.ensemble import RandomForestRegressor
    from sklearn.neural_network import MLPRegressor
    import lightgbm as lgb
    name, fs = winner.rsplit("_", 1)
    Xtr = feats.design_matrix(train, fs=="lags"); Xte = feats.design_matrix(test, fs=="lags")
    if drop_cols:
...
def main():
    ...
    winner = sig["best_ml"]; params = eval(tuning["best_params"][winner])
    df = feats.load_panel()
    tr0, te0 = feats.split(df, O3)

    checks = []
    def run(label, train, test, ml_kw=None, pp_kw=None, params_override=None):
        p_ml = fit_ml(train, test, winner, params_override or params, **(ml_kw or {}))
