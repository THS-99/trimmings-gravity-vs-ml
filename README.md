...
- Estimating the gravity model in two specifications, one interpretable, with year and heading fixed effects and standard errors clustered by exporter-destination pair, and one for prediction, with exporter, destination and product fixed effects instead of year effects so that it can be applied to a year the fit has not seen.
...
- Running the robustness checks: without heading 5806, at HS4 level, without the pandemic years, with perturbed hyperparameters, without the lag features, without the agreement dummy and with the EU as one bloc.

Algorithms used: Poisson pseudo maximum likelihood with fixed effects for the gravity side, random forest, gradient boosting with LightGBM and a multilayer perceptron for the machine learning side, a naive persistence forecast (last year's value) as the reference point for both, blocked forward validation by year for the train and test split (the panel is a time series, so a random split would leak future information into training), paired bootstrap over exporter-destination pairs for the significance tests between models (rows of the same corridor are not independent, so the pair is the resampling unit), permutation importance and SHAP values for the explainability part. The gravity prediction that feeds the hybrid models is always a one-year-ahead prediction from a fit on the earlier years, for training and test rows alike, and the number of training epochs of the perceptron is chosen on the last training year rather than on a random validation split.
...
How to run: `bash code/run_all.sh` from the repository root runs the numbered scripts in `code/` in order (07 is a module the others import) and writes every table and figure to `results/`. The download scripts 01 to 04 need network access and run the queries logged in `data/DOWNLOAD_LOG.md`; the raw files land in `data/raw/`. Scripts 09 and 13 reuse their own outputs when they find them (`results/ml_tuning_report.json` and `results/table_5_5_robustness.csv`), so delete those two files first to force a full recomputation. The notebook `code/ML_trimmings_thesis.ipynb` is a lighter version of scripts 07 to 12 that loads the compressed panel in `data/processed/` directly and runs in about fifteen minutes on a free Colab runtime.
