"""Build the exporter by destination by HS6 by year panel from the four downloads."""
import json
import pandas as pd
import numpy as np
...
def main():
    ...
    for y, g, p in tw.itertuples(index=False):
        ...
        panel.loc[m, "gdp_exporter"] = panel.loc[m, "gdp_exporter"].fillna(g*1e3)
        panel.loc[m, "pop_exporter"] = panel.loc[m, "pop_exporter"].fillna(p*1e3)
