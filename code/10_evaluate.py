...
def main():
    ...
    # the ML itself? Same paired bootstrap, best hybrid vs best pure ML.
    hyb = pooled[pooled.model.str.contains("hyb")]
    if len(hyb):
        best_hyb = hyb.iloc[0].model
        pure = pooled[~pooled.model.str.startswith("PPML") & ~pooled.model.str.contains("hyb")]
        ...
        e_h, e_p = (pair2.y_pred-pair2.y_true)**2, (pair2.y_pred_pure-pair2.y_true)**2
        obs2 = float(np.sqrt(e_p.mean()) - np.sqrt(e_h.mean()))
        boots2 = np.empty(2000)
        idx2 = np.arange(len(pair2))
        for i in range(2000):
            s = RNG.choice(idx2, len(pair2), replace=True)
            boots2[i] = np.sqrt(e_p.values[s].mean()) - np.sqrt(e_h.values[s].mean())
        ci2 = np.percentile(boots2, [2.5, 97.5])
        p2 = float(2*min((boots2<=0).mean(), (boots2>=0).mean()))
        sig["hybrid_vs_pure"] = {
